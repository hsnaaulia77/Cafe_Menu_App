        ...
        <div class="col-md-7">
            <div class="card shadow-lg animate__animated animate__fadeInUp" style="border-radius: 18px;">
                <div class="card-header card-header-coffee" style="border-radius: 18px 18px 0 0;">
                    <h3 class="card-title mb-0"><i class="fas fa-edit mr-2"></i> Edit Kategori Menu</h3>
                </div>
                <form action="{{ route('admin.kategori_menu.update', $kategori->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="form-group mb-4">
                            <label for="nama" class="form-label font-weight-bold">Nama Kategori <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama" value="{{ old('nama', $kategori->nama) }}" required autofocus style="border-radius:8px;">
                            @error('nama')
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="3" style="border-radius:8px;">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="card-footer d-flex justify-content-between align-items-center" style="border-radius: 0 0 18px 18px;">
                        ...
                        <button type="submit" class="btn btn-primary btn-lg shadow" data-toggle="tooltip" title="Update kategori">
